...
export default function Home() {
  ...
  async function generate() {
    if (!key.trim()) { setErr("Please add your API Key first!"); setShowK(true); return; }
    ...
    const args = { p: prompt, n: neg, img: mode === "image" ? img : undefined, ar };
      ...
      <div style={S.app}>
        <div style={S.wrap}>
          ...
          {showK && (
            <div style={{ ...S.card, borderColor: "rgba(255,80,80,0.2)" }}>
              <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: "#ff7070" }}>🔑 fal.ai API Key</span>
                <a href="https://fal.ai/dashboard/keys" target="_blank" rel="noreferrer" style={{ marginLeft: "auto", fontSize: 12, color: "#00f5d4", textDecoration: "none" }}>Get Key →</a>
              </div>
              <input type="password" placeholder="fal_xxx..." value={key} onChange={e => setKey(e.target.value)}
              ...
              <p style={{ fontSize: 11, color: "#555", marginTop: 7 }}>🔒 Key is stored only in this session.</p>
            </div>
          )}
          ...
          <div style={S.card}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
                <div style={S.lbl}>PROMPT</div>
                <span style={{ fontSize: 12, color: "#00f5d4", fontWeight: 700 }}>{words} words</span>
              </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <select value={ar} onChange={e => setAr(e.target.value)} style={{ padding: "9px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, color: "#ccc", fontSize: 13, cursor: "pointer", outline: "none" }}>
            ...
            <button style={S.bgen(loading)} onClick={generate} disabled={loading}>
              {loading ? "⏳ Generating..." : `⚡ Generate ${model?.maxSec}s Video`}
            </button>
          </div>
          ...
          {loading && (
            <div style={{ ...S.card, textAlign: "center" }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 20, height: 5, overflow: "hidden", maxWidth: 300, margin: "0 auto" }}>
                <div style={{ height: "100%", background: "linear-gradient(90deg,#00f5d4,#7928ca)", width: `${pct}%`, transition: "width 1s" }} />
              </div>
            </div>
          )}
            ...
            <div>
              {clips.map((c, i) => (
                <div key={i} style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 12 }}>
                  ...
                  <video src={c.url} controls autoPlay loop style={{ width: "100%", background: "#000", display: "block" }} />
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ marginTop: 32, padding: "16px 20px", borderRadius: 12, background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.04)", fontSize: 11, color: "#333", textAlign: "center" }}>
            VGENAI Studio · Powered by fal.ai
          </div>
        </div>
      </div>
